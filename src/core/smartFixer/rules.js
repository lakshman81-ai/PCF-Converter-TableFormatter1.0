...
export function runElementRules(element, context, prevElement, elemAxis, elemDir, config, log) {
  ...
  if (type === "PIPE") {
    ...
    if (len < (cfg.microPipeThreshold ?? 6.0)) {
      ...
      log.push({ type: "Fix", stage: 3, ruleId: "R-GEO-01", tier: 1, row: ri,
  ...
  if (prevElement && element.bore && prevElement.bore && element.bore !== prevElement.bore) {
    if (type !== "REDUCER-CONCENTRIC" && type !== "REDUCER-ECCENTRIC" &&
      ...
      log.push({ type: "Error", stage: 3, ruleId: "R-GEO-02", tier: 4, row: ri,
  ...
  if (type === "PIPE" && element.ep1 && element.ep2) {
    ...
    if (nonZeros.length >= 2) {
      ...
      if (nonZeros[1].val < (cfg.silentSnapThreshold ?? 2.0)) {
         ...
         log.push({ type: "Fix", stage: 3, ruleId: "R-GEO-03", tier: 2, row: ri,
           message: `SNAP [R-GEO-03]: Pipe has ${nonZeros[1].val.toFixed(1)}mm off-axis drift. Snapping to ${nonZeros[0].axis}.` });
      } else {
         log.push({ type: "Error", stage: 3, ruleId: "R-GEO-03", tier: 4, row: ri,
  ...
  if (prevElement && type !== "BEND" && context.travelAxis && elemAxis && elemAxis !== context.travelAxis) {
    if (type === "PIPE" && (prevElement.type || "").toUpperCase() === "PIPE") {
       ...
       log.push({ type: "Error", stage: 3, ruleId: "R-CHN-01", tier: 4, row: ri,
  ...
  if (prevElement && context.travelAxis && elemAxis === context.travelAxis && elemDir === -context.travelDirection) {
    ...
    if (type === "PIPE") {
      ...
      if (foldLen <= (cfg.autoDeleteFoldbackMax ?? 25.0)) {
         ...
         log.push({ type: "Fix", stage: 3, ruleId: "R-CHN-02", tier: 2, row: ri,
           message: `DELETE [R-CHN-02]: Fold-back pipe ${foldLen.toFixed(1)}mm.` });
      } else {
         log.push({ type: "Error", stage: 3, ruleId: "R-CHN-02", tier: 4, row: ri,
    ...
    if (exitPt && entryPt) {
      ...
      for (const axis of nonTravelAxes) {
        ...
        if (drift > 0.1 && drift < silentSnap) {
          ...
          log.push({ type: "Fix", stage: 3, ruleId: "R-CHN-06", tier: 1, row: ri,
          ...
          log.push({ type: "Fix", stage: 3, ruleId: "R-CHN-06", tier: 2, row: ri,
          ...
          logRuleExecution("R-CHN-06", ri);
          log.push({ type: "Error", stage: 3, ruleId: "R-CHN-06", tier: 4, row: ri,
  ...
  for (const { name, val } of fields) {
    ...
    if (val.x === 0 && val.y === 0 && val.z === 0) {
      ...
      log.push({ type: "Error", stage: 3, ruleId: "R-GEO-08", tier: 4, row: ri,
    ...
    for (const axis of ["x", "y", "z"]) {
      if (Math.abs(val[axis]) > 500000) {
        logRuleExecution("R-GEO-08", ri);
        log.push({ type: "Warning", stage: 3, ruleId: "R-GEO-08", tier: 3, row: ri,
  ...
  if (type === "TEE" && element.branchBore > element.bore) {
    ...
    log.push({ type: "Error", stage: 3, ruleId: "R-BRN-01", tier: 4, row: ri, message: `ERROR [R-BRN-01]: Branch bore > header bore.` });
  }
  ...
  if (type === "OLET" && element.branchBore && element.bore) {
    ...
    if (ratio > (cfg.oletMaxRatioError ?? 0.8)) {
       log.push({ type: "Error", stage: 3, ruleId: "R-BRN-02", tier: 4, row: ri, message: `ERROR [R-BRN-02]: OLET ratio > 0.8.` });
    } else if (ratio > (cfg.oletMaxRatioWarning ?? 0.5)) {
       log.push({ type: "Warning", stage: 3, ruleId: "R-BRN-02", tier: 3, row: ri, message: `WARNING [R-BRN-02]: OLET ratio > 0.5.` });
    }
  }
  ...
  if (type === "TEE" && context.travelAxis && element.branchBore) {
      ...
      if (element.bp && element.cp) {
          ...
          const bpNonZero = [["X", bpVec.x], ["Y", bpVec.y], ["Z", bpVec.z]].filter(([, d]) => Math.abs(d) > 0.5);
          if (bpNonZero.length > 0) {
              ...
              if (bpDom.toUpperCase() === context.travelAxis) {
                 log.push({ type: "Error", stage: 3, ruleId: "R-BRN-03", tier: 4, row: ri, message: `ERROR [R-BRN-03]: Branch same axis as header.` });
              }
          }
      }
  }
  ...
  if (type === "TEE" && element.bp && element.cp && element.ep1 && element.ep2) {
      ...
      if (hMag > 0.1 && bMag > 0.1) {
         ...
         if (offPerp > errThreshold) {
            ...
            log.push({ type: "Error", stage: 3, ruleId: "R-BRN-04", tier: 4, row: ri,
            ...
            logRuleExecution("R-BRN-04", ri);
            log.push({ type: "Warning", stage: 3, ruleId: "R-BRN-04", tier: 3, row: ri,
  ...
  if (type === "BEND" && context.lastFittingType === "BEND") {
      ...
      if (context.pipeSinceLastBend === 0) {
          log.push({ type: "Warning", stage: 3, ruleId: "R-CHN-03", tier: 3, row: ri, message: `WARNING [R-CHN-03]: Two bends with 0mm pipe between.` });
      }
  }
  ...
  if (element.ep1 && element.ep2 && type !== "SUPPORT" && type !== "OLET") {
      ...
      if (vec.mag(vec.sub(element.ep2, element.ep1)) === 0) {
          log.push({ type: "Error", stage: 3, ruleId: "R-GEO-07", tier: 4, row: ri, message: `ERROR [R-GEO-07]: Zero-length element.` });
      }
  }
  ...
  if (element.ep1) {
    ...
    if (decX !== cfg.decimals || decY !== cfg.decimals || decZ !== cfg.decimals) {
      ...
      log.push({ type: "Warning", stage: 3, ruleId: "R-DAT-01", tier: 3, row: ri, message: `WARNING [R-DAT-01]: Precision mismatch.` });
    }
  }
  ...
  if (element.ca && element.ca[3] && context.currentMaterial && element.ca[3] !== context.currentMaterial) {
    if (!["FLANGE", "VALVE"].includes(prevElement?.type?.toUpperCase())) {
      ...
      log.push({ type: "Warning", stage: 3, ruleId: "R-DAT-03", tier: 3, row: ri, message: `WARNING [R-DAT-03]: Material changed without joint.` });
    }
  }
  ...
  if (element.ca && element.ca[1] && context.currentPressure && element.ca[1] !== context.currentPressure) {
    ...
    log.push({ type: "Warning", stage: 3, ruleId: "R-DAT-04", tier: 3, row: ri, message: `WARNING [R-DAT-04]: Pressure changed.` });
  }

  // R-DAT-05: CA8 Weight Scope
  if (element.ca && element.ca[8] && ["PIPE", "SUPPORT"].includes(type)) {
     logRuleExecution("R-DAT-05", ri);
     log.push({ type: "Warning", stage: 3, ruleId: "R-DAT-05", tier: 3, row: ri, message: `WARNING [R-DAT-05]: CA8 (weight) on PIPE/SUPPORT.` });
  }
  ...
  if (element.skey) {
    ...
    if (expected && !expected.some(p => element.skey.toUpperCase().startsWith(p))) {
      ...
      log.push({ type: "Warning", stage: 3, ruleId: "R-DAT-06", tier: 3, row: ri, message: `WARNING [R-DAT-06]: SKEY ${element.skey} mismatch for ${type}.` });
    }
  }
...
export function runSupportRules(support, chain, context, config, log) {
  ...
  if (minDist > 5.0 && minDist < Infinity) {
    log.push({ type: "Error", stage: 3, ruleId: "R-TOP-06", tier: 4, row: ri,
...
export function runAggregateRules(chain, context, config, log) {
  ...
  if (lastElement && (lastElement.type || "").toUpperCase() === "PIPE") {
     ...
     log.push({ type: "Warning", stage: 3, ruleId: "R-TOP-01", tier: 3, row: lastElement._rowIndex, message: `WARNING [R-TOP-01]: Chain ends at bare pipe.` });
  }
  ...
  if (flanges.length % 2 !== 0 && flanges.length > 1) {
      ...
      log.push({ type: "Warning", stage: 3, ruleId: "R-AGG-05", tier: 3, row: ri, message: `WARNING [R-AGG-05]: Odd number of flanges in chain.` });
  }

  // Also manually tag R-TOP-04 for single mid-chain flange
  if (flanges.length === 1 && chain.length > 2) {
      logRuleExecution("R-TOP-04", ri);
      log.push({ type: "Warning", stage: 3, ruleId: "R-TOP-04", tier: 3, row: flanges[0].element._rowIndex, message: `WARNING [R-TOP-04]: Single mid-chain flange.` });
  }
  ...
  if (chain.length <= 2 && chain.every(l => (l.element.type || "").toUpperCase() !== "PIPE")) {
     ...
     log.push({ type: "Warning", stage: 3, ruleId: "R-AGG-06", tier: 3, row: ri, message: `WARNING [R-AGG-06]: Chain has only fittings, no pipe.` });
  }

  if (context.pipeLengthSum <= 0 && chain.length > 0) {
    logRuleExecution("R-AGG-01", ri);
    log.push({ type: "Error", stage: 3, ruleId: "R-AGG-01", tier: 4, row: ri,
  ...
  if (chain.length >= 2) {
    ...
    if (startPt && endPt) {
      ...
      if (error > (cfg.closureErrorThreshold ?? 50.0)) {
        ...
        log.push({ type: "Error", stage: 3, ruleId: "R-AGG-03", tier: 4, row: ri,
  ...
  for (let i = 0; i < components.length; i++) {
    ...
    for (let j = i + 1; j < components.length; j++) {
      ...
      if (aType === bType &&
        ...
        log.push({ type: "Error", stage: 3, ruleId: "R-TOP-03", tier: 4, row: b._rowIndex,
...
export function detectOrphans(components, visited, log) {
  ...
  for (const orphan of orphans) {
    ...
    log.push({ type: "Error", stage: 3, ruleId: "R-TOP-02", tier: 4, row: orphan._rowIndex,
...
export function applyRSPA02(chains, config, log) {
          ...
          for (let j = runStart; j <= runEnd; j++) {
            ...
            for (const pt of ["ep1", "ep2"]) {
              ...
              if (drift > 0.1 && drift < silentSnap) {
                ...
                log.push({ type: "Fix", stage: 3, ruleId: "R-SPA-02", tier: 2, row: elem._rowIndex,
                  message: `SNAP [R-SPA-02 T2]: ${axis.toUpperCase()} drifted ${drift.toFixed(1)}mm from run median ${median.toFixed(1)}. Snapped.` });
              } else if (drift >= warnSnap) {
                logRuleExecution("R-SPA-02", elem._rowIndex);
                log.push({ type: "Error", stage: 3, ruleId: "R-SPA-02", tier: 4, row: elem._rowIndex,
  ...
  log.push({ type: "Info", stage: 3, message: `R-SPA-02: Snapped ${snapCount} coordinates across all chains.` });
}
...
export function checkRBRN05(teeElement, branchChain, config, log) {
  ...
  if (gap > tolerance) {
    ...
    log.push({ type: "Error", stage: 3, ruleId: "R-BRN-05", tier: 4, row: branchFirst._rowIndex,
  ...
  if (branchFirst.bore && teeElement.branchBore &&
    ...
    logRuleExecution("R-BRN-05", branchFirst._rowIndex);
    log.push({ type: "Warning", stage: 3, ruleId: "R-BRN-05", tier: 3, row: branchFirst._rowIndex,
...
function snapElevation(zValues, chain, silentSnap, warnSnap, log) {
  ...
  for (const entry of zValues) {
    ...
    if (drift > 0.1 && drift < silentSnap) {
      ...
      log.push({ type: "Fix", stage: 3, ruleId: "R-SPA-01", tier: 1, row: elem._rowIndex,
      ...
      logRuleExecution("R-SPA-01", elem._rowIndex);
      log.push({ type: "Fix", stage: 3, ruleId: "R-SPA-01", tier: 2, row: elem._rowIndex,
        message: `SNAP [R-SPA-01 T2]: Elevation Z drifted ${drift.toFixed(1)}mm from horizontal run median ${median.toFixed(1)}. Snapped.` });
    } else if (drift >= warnSnap) {
      logRuleExecution("R-SPA-01", elem._rowIndex);
      log.push({ type: "Warning", stage: 3, ruleId: "R-SPA-01", tier: 3, row: elem._rowIndex,
...
export function checkROVR05(current, next, overlapAmt, context, config, log) {
  ...
  if (halfC && Math.abs(overlapAmt - halfC) < 3.0) {
    ...
    log.push({ type: "Fix", stage: 3, ruleId: "R-OVR-05", tier: 2, row: pipe._rowIndex,
  ...
  logRuleExecution("R-OVR-05", pipe._rowIndex);
  log.push({ type: "Warning", stage: 3, ruleId: "R-OVR-05", tier: 3, row: pipe._rowIndex,
